...
const Signup = () => {
    ...
    return <Container>
        ...
        <Input onChange={(event) => { setRepeatedPassword(event.target.value) }} placeholder="repeat your password" type="password"></Input>
        {error && <Paragraph error centered>
            {error}
        </Paragraph>}
        <h2>2/2 - Your payment details</h2>
        <Input onChange={() => { throw "not implemented" }} placeholder="card number" type="text"></Input>
        <Input onChange={() => { throw "not implemented" }} placeholder="name on card" type="text"></Input>
        <Input onChange={() => { throw "not implemented" }} placeholder="CVC" type="text"></Input>
        <Button onClick={onSignup} disabled={!(email && password)} >Completed</Button>
    </Container>
}
