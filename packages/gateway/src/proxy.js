...
var withProxies = function (pairs, app) {
    ...
    pairs.forEach(function (_a) {
        var path = _a[0], target = _a[1];
        return createProxy(app)(path, target);
    });
    return app;
};
exports.withProxies = withProxies;
var mapping = function (path, hostname, port) { return [path, "http://" + hostname + ":" + port]; };
exports.mapping = mapping;
var createProxy = function (handler) { return function (path, target) {
    handler.use(path, http_proxy_middleware_1.createProxyMiddleware({
        target: target,
