...
export const withMockedSchemaVariable =
  async (schema: string | undefined, action: () => void | Promise<void>) =>
  async () => {
...
describe("The database module", () => {
  ...
  it("Throws if the schema name is not defined", () => {
    ...
    withMockedSchemaVariable(schema, () => {
      expect(() => database()).rejects.toThrow(
        `Schema needs to be defined in env: undefined`
      );
    });
  });
