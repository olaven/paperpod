...
import { withCollection, ifCloud, port, urlSuffix, connectionString } from "./withCollection";
...
describe("Higher order function for getting a mongo collection", () => {
    ...
    it("Is possible to use 'withCollection' to interact with mongo", async () => {
        ...
        await withTestCollection(collection =>
            collection.insertOne(document)
        );

        const retrieved = await withTestCollection(collection => collection.findOne({ a: document.a }));
    ...
    describe("Helper functions for 'withCollection'", () => {
        ...
        const withCloudProtocol = withMockedEnv({ MONGODB_PROTOCOL: "mongodb+srv" });
        ...
        describe("function generating port section of connection string", () => {
            ...
            const withMongoPort = withMockedEnv({ MONGODB_PORT: faker.random.number({ min: 1000, max: 9000 }) });
